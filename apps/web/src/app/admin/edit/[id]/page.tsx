...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Products API lists by slug but admin needs by ID — use list with search as fallback
        // The backend also exposes GET /products/:slug so we fetch all and find by id
        ...
        reset({
          ...
          category: found.category as 'Men' | 'Women' | 'Kids',
          subCategory: found.subCategory as 'Topwear' | 'Bottomwear' | 'Winterwear',
          isBestseller: found.isBestseller,
          imageUrls,
        });

        const newSizeMap = Object.fromEntries(
          SIZES.map((s) => [s, { enabled: false, stock: 0 }]),
        ) as Record<SizeKey, SizeEntry>;
        found.variants.forEach((v) => {
          if (SIZES.includes(v.size as SizeKey)) {
            newSizeMap[v.size as SizeKey] = { enabled: true, stock: v.stock };
          }
        });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-3xl space-y-8">
      <button
        type="button"
        onClick={() => router.back()}
        className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-[#111] transition-colors mb-2"
      >
        <ArrowLeft size={14} />
        Back to Products
      </button>
      ...
      <div className="bg-white rounded-lg border border-gray-100 p-6 space-y-4">
        <h2 className="text-sm font-bold text-[#111] uppercase tracking-wide">Sizes &amp; Stock</h2>
